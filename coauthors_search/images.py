...
_DEFAULT_CONFIGS = {"graph_name": "Authors",
                    "format": "pdf",
                    'additional_nodes': 0,
...
def generate_graph2(tree: Tree, target: Author, *, configuration: Dict[str, Any]):
    graph = None
...
def generate_graph(tree: Tree, target: Author, configuration: Dict[str, Any]):
    ...
    try:
        ...
        raise ValueError("Missing requirement configuration: 'graph type'")

    try:
        img_path = configuration["image_path"]
    except KeyError:
        raise ValueError("Missing requirement configuration: 'image_path'")

    try:
        graph_path = configuration["graph_path"]
    except KeyError:
        raise ValueError("Missing requirement configuration: 'graph_path'")
